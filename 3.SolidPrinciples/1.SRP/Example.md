...
> so what have we done so far we only kept calculate total method in Invoice class  and seperated the other method like saveToDb to another data access class  and generateInvoice to another class  now each of the class has only single reason to change the class 
